...
export default function RockPaperScissors({ logWager }: RpsProps) {
    ...
    const handlePlaySubmit = async () => {
        ...
        try {
            ...
            // 🔒 C-8 FIX: fetch the House's commitment BEFORE signing the play tx.
            // The hash is generated by the backend without knowledge of `currentMove`,
            // so the House cannot bias its move based on the player's choice. The
            // hash is then bound into the play_hand tx itself.
            const commitRes = await fetch(`${BACKEND_URL}/api/rps/commitment`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ playerPubkey: publicKey.toBase58() })
            });
            const commitData = await commitRes.json();
            if (!commitData.success || !commitData.hashedCommitment) {
                throw new Error(`House failed to generate commitment: ${commitData.error || 'unknown'}`);
            }
            const hashedCommitmentBytes = Array.from(Buffer.from(commitData.hashedCommitment, 'hex'));
            if (hashedCommitmentBytes.length !== 32) {
                throw new Error("House returned malformed commitment.");
            }
            ...
            const playIx = await program.methods.rpsPlayHand(lamports, currentMove, hashedCommitmentBytes)
